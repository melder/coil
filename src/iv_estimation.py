"""
Module for fetching and processing near-the-money options data using the hood wrapper
and computing custom implied volatilities via Black-Scholes inversion.
"""
...
def get_nearest_atm_options(ticker: str, risk_free_rate: float = 0.045):
    ...
    if not current_price:
        logging.error("Could not fetch price for %s", ticker)
        return None

    expirations = hood.get_expirations(ticker)
    if not expirations:
        logging.error("Could not fetch expirations for %s", ticker)
        return None
    ...
    if not valid_exps:
        logging.warning("No future expirations found for %s", ticker)
        return None
    ...
    logging.info(
        "%s @ %.2f | Nearest Exp: %s | DTE: %.4f days",
        ticker,
        current_price,
        nearest_exp,
        time_to_exp_years * 365.25,
    )
    ...
    if not calls or not puts:
        logging.error("Failed to fetch option chain for %s @ %s", ticker, nearest_exp)
        return None
...
def calculate_custom_iv(S, K, T, r, market_price, option_type="call"):
    ...
    try:
        ...
        # FIXED: Enclosed multiple exceptions in a tuple for modern Python compatibility
        return np.nan
...
if __name__ == "__main__":
    try:
        ...
        if result:
            ...
            print(
                f"Price: ${result['current_price']:.2f} | Exp: {result['expiration']} (DTE: {result['dte_years'] * 365.25:.4f} days)\n"
            )
            for label, opt in result["contracts"].items():
                if not opt:
                    continue
                print(
                    f"[{label.upper():10}] Strike: {opt['strike']:6.1f} | Mark: {opt['mark']:6.2f} | Bid/Ask: {opt['bid']:.2f}/{opt['ask']:.2f} (Spread Score: {opt['spread_score']:5.1f}%)"
                )
                print(
                    f"             Robinhood IV: {opt['rh_iv']:.4f} | Custom IV: {opt['custom_iv']:.4f}\n"
                )
