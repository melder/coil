...
def login():
    ...
    Optionally uses RH_2FA_SECRET for TOTP and RH_PICKLE_NAME for session caching.
    """
    ...
    if not username or not password:
        ...
        raise HoodException("Missing Robinhood credentials.")
...
def get_expirations(ticker):
    try:
        return rh.options.get_options_expirations(ticker)
    except Exception as e:
        logging.error(f"Failed to get expirations for {ticker}: {e}")
        return []
...
def get_options_by_expiration(ticker, expr, option_type=None):
    try:
        ...
        logging.error(f"Unexpected {err=}, {type(err)=}. Failed to get option chain data for {ticker}")
        return []
...
def get_market_hours(iso_date):
    try:
        ...
        logging.error(f"Failed to get market hours for {iso_date}: {e}")
        return None
...
def absolute_seconds_until_expr(iso_date):
    ...
    if not close_dt:
        ...
        logging.warning(f"Market hours missing for {iso_date}, defaulting to 16:00 EST ({close_dt})")
...
def get_dte_years(iso_date):
    """
    Returns precise Time to Expiration in Years based on absolute market seconds.
    """
